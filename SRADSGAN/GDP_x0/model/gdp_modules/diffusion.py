...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None):
        if condition_x is not None:
            x_recon = self.denoise_fn(torch.cat([x, condition_x], dim=1), t)
        else:
    ...
    def p_losses(self, x_in, noise=None):
        ...
        if not self.conditional:
            ...
            x_recon = self.denoise_fn(torch.cat([x_t, x_sr], dim=1), t)

        #x_recon_degradation = trans_fn.resize(x_recon, x_lr.shape[3], Image.BICUBIC)

        #loss = self.loss_func(x_recon_degradation, x_lr)
