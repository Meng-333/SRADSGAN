...
class RAB(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, padding=1, bias=True, dilation=1, act_type='lrelu',
        ...
        if self.la_mode.find('CA') != -1:
            self.ca = CLAM(planes, pool_mode=pool_mode)
        ...
        if self.la_mode == '':
            self.last_conv = nn.Conv2d(64, 64, kernel_size=1, bias=True)
...
class ResGroup(nn.Module):
    def __init__(self, block, n_blocks=5, nc=64, kernel_size=3, stride=1, bias=True, padding=1,
        ...
        self.addconv = addconv
        if self.la_mode.find('CA') != -1:
            self.ca = CLAM(nc, pool_mode=pool_mode)
        if self.la_mode.find('SA') != -1:
            self.sa = SLAM(kernel_size=7, pool_mode=pool_mode)
        if self.la_mode.find('|') != -1:
...
class ACB(nn.Module):
    def __init__(self, inplanes, planes):
        ...
        self.inplanes = inplanes
        self.planes = planes
        self.conv1x3 = torch.nn.Conv2d(inplanes, planes, (1, 3), 1, (0, 1))
        self.conv3x1 = torch.nn.Conv2d(inplanes, planes, (3, 1), 1, (1, 0))
        self.conv3x3 = torch.nn.Conv2d(inplanes, planes, (3, 3), 1, (1, 1))
...
class GeneratorResNet(nn.Module):
    def __init__(self, buildingblock, in_channels=3, out_channels=3, n_residual_blocks=12, n_basic_blocks=3,
        ...
        for i in range(n_residual_blocks):
            layer.append(buildingblock(RAB, n_blocks=n_basic_blocks, nc=64, kernel_size=3, stride=1, padding=1,
        ...
        # ACNet - ACBlock     #https://github.com/DingXiaoH/ACNet
        # self.ACB = ACB(inplanes=in_channels, planes=64)
    ...
    def forward(self, x):
        ...
        # # no dense sample
        # out_all = self.conv1(x)
        # out_all = self.res_groups(out_all)
...
class SRADSGAN(object):
    ...
    def train(self):
        ...
        self.generator = GeneratorResNet(ResGroup, n_residual_blocks=12, n_basic_blocks=3,   #默认n_residual_blocks=12  n_basic_blocks=5
        ...
        if self.epoch != 0:
            ...
            # G_model_path = '/home/zju/mfe/ESRRDAN/Result/model/sradsgan_x2_generator_param.pkl'
            # D_model_path = '/home/zju/mfe/ESRRDAN/Result/model/sradsgan_x2_discriminator_param.pkl'
    ...
    def mfeNew_validate(self, epoch=100, modelpath=None):
        ...
        print_rlt['bicubic_ergas'] = avg_bicubic_ergas
        print_rlt['bicubic_lpips'] = avg_bicubic_lpips
        print_rlt['sradsgan_mse'] = avg_srcnn_mse
        print_rlt['sradsgan_psnr'] = avg_srcnn_psnr
        print_rlt['sradsgan_ssim'] = avg_srcnn_ssim
        print_rlt['sradsgan_ergas'] = avg_srcnn_ergas
        print_rlt['sradsgan_lpips'] = avg_srcnn_lpips
        self.logger.print_format_results('val', print_rlt)
        print('-----------------------------------')
        del test_data_loader
        torch.cuda.empty_cache()
    ...
    def mfeNew_validateByClass(self, epoch, save_img=False, modelpath=None):
        ...
        if modelpath is not None:
            self.generator.load_state_dict(torch.load(modelpath), strict=False)
        srutils.print_network_to_file(self.generator, save_dir=self.save_dir, tag='SRADSGAN')
        ...
        print_rlt['sradsgan_mse'] = average_srrgan_mse
        print_rlt['sradsgan_psnr'] = average_srrgan_psnr
        print_rlt['sradsgan_ssim'] = average_srrgan_ssim
        print_rlt['sradsgan_ergas'] = average_srrgan_ergas
        print_rlt['sradsgan_lpips'] = average_srrgan_lpips
        self.logger.print_format_results('val', print_rlt)
        print('-----------------------------------')
        torch.cuda.empty_cache()
    ...
    def mfe_test_single(self, img_fn, modelpath=None):
        ...
        srutils.save_img1(recon_imgs[0].cpu().data, self.save_dir, self.save_dir+'/SR_SRADSGAN_%s' % img_name)
