...
class GeneratorResNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_residual_blocks=16, upscale_factor=3):
...
class SRGAN(object):
    ...
    def train(self):
        ...
        self.generator = GeneratorResNet(in_channels=3, out_channels=3, n_residual_blocks=16, upscale_factor=self.scale_factor)
        self.discriminator = Discriminator()
        self.feature_extractor = FeatureExtractor()

        # Losses
        self.criterion_GAN = torch.nn.MSELoss()
        # self.criterion_content = torch.nn.L1Loss()   # MAE
        self.criterion_content = torch.nn.MSELoss()   # MSE
        ...
        for epoch in range(self.epoch, self.num_epochs):
            ...
            for i, (input, target, bc_input, imgpath) in enumerate(dataloader):
                ...
                loss_G = mse_loss_G + 6e-3 * loss_content + 1e-3 * loss_GAN
    ...
    def mfeNew_validate(self, epoch=100, modelpath=None):
        ...
        print_rlt['bicubic_ergas'] = avg_bicubic_ergas
        print_rlt['bicubic_lpips'] = avg_bicubic_lpips
        print_rlt['srgan_mse'] = avg_srcnn_mse
        print_rlt['srgan_psnr'] = avg_srcnn_psnr
        print_rlt['srgan_ssim'] = avg_srcnn_ssim
        print_rlt['srgan_ergas'] = avg_srcnn_ergas
        print_rlt['srgan_lpips'] = avg_srcnn_lpips
        self.logger.print_format_results('val', print_rlt)
        print('-----------------------------------')
    ...
    def mfeNew_validateByClass(self, epoch, save_img=False, modelpath=None):
        ...
        if modelpath is not None:
            self.generator.load_state_dict(torch.load(modelpath), strict=False)
        srutils.print_network_to_file(self.generator, save_dir=self.save_dir, tag='SRGAN')
        ...
        print_rlt['srgan_mse'] = average_srrgan_mse
        print_rlt['srgan_psnr'] = average_srrgan_psnr
        print_rlt['srgan_ssim'] = average_srrgan_ssim
        print_rlt['srgan_ergas'] = average_srrgan_ergas
        print_rlt['srgan_lpips'] = average_srrgan_lpips
        self.logger.print_format_results('val', print_rlt)
        print('-----------------------------------')
        torch.cuda.empty_cache()
    ...
    def mfe_test_single(self, img_fn, modelpath=None):
        ...
        srutils.save_img1(recon_imgs[0].cpu().data, self.save_dir, self.save_dir+'/SR_SRGAN_%s' % img_name)
